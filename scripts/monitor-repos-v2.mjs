...
import { applyBenchmark, auditProject } from "./project-audit.mjs";
...
function assertSafeStudentId(studentId) {
  if (!/^[a-z0-9-]+$/i.test(studentId)) throw new Error("Identificador de estudiante no seguro");
}
...
async function scanProject(directory) {
  ...
  async function walk(currentDirectory) {
    ...
    for (const entry of entries) {
      ...
      if (files.length > MAX_PROJECT_FILES) throw new Error(`El repositorio supera ${MAX_PROJECT_FILES} archivos`);
      if (totalBytes > MAX_PROJECT_BYTES) throw new Error("El repositorio supera el límite local de 50 MB");
    }
  }
...
function scoreEntryPath(filePath) {
  ...
  if (normalized.includes("indexcarrito")) return -1100;
  if (normalized.includes("delivery")) return -1050;
  if (normalized.includes("primera")) return -1000;
  if (normalized.includes("atm") && normalized.endsWith("/index.html")) return -980;
  if (normalized === "index.html") return -900;
  if (normalized.endsWith("/index.html")) return -800 + filePath.split("/").length;
  if (normalized.includes("formulario")) return -750;
  return filePath.split("/").length * 20 + filePath.length;
}

async function buildManifest(student, latestCommit, currentPath, scan, mirroredAt) {
...
async function mirrorRepository(student, latestCommit) {
  ...
  if (existingManifest?.commit?.sha === latestCommit.sha && await pathExists(currentPath)) {
    const scan = await scanProject(currentPath);
    const manifest = await buildManifest(
      student,
      latestCommit,
      currentPath,
      scan,
      existingManifest.mirroredAt || new Date().toISOString()
    );
  ...
  try {
    ...
    if (checkedOutSha !== latestCommit.sha) throw new Error("El SHA descargado no coincide con el commit monitoreado");

    await fs.rm(path.join(tempPath, ".git"), { recursive: true, force: true });
    const scan = await scanProject(tempPath);
    const mirroredAt = new Date().toISOString();
    const manifest = await buildManifest(student, latestCommit, tempPath, scan, mirroredAt);
...
for (const student of roster.students) {
  ...
  try {
    ...
    try {
      ...
      if (mirror.audit) result.auditsReady += 1;
    } catch (mirrorError) {
      mirror = {
        status: "error",
    ...
    result.students[student.id] = {
      ...
      mirror: {
        ...
        auditScore: mirror.audit?.score || null,
...
applyBenchmark(projectIndex);

for (const [studentId, project] of Object.entries(projectIndex.projects)) {
  if (result.students[studentId]?.mirror && project.audit?.benchmark) {
    result.students[studentId].mirror.benchmark = project.audit.benchmark;
  }
}
...
const benchmark = projectIndex.benchmark;
const summaryLines = [
  ...
  `- Auditorías técnicas listas: ${result.auditsReady}`,
  `- Actualizaciones detectadas: ${result.updatesDetected}`,
  `- Proyecto de referencia: ${benchmark ? `${benchmark.name} — ${benchmark.percent}%` : "pendiente"}`,
  "",
  "| Grupo | Estudiante | Repositorio | Copia | Puntaje técnico | Último commit |",
  "|---|---|---|---|---:|---|",
  ...roster.students.map((student) => {
    const entry = result.students[student.id];
    const project = projectIndex.projects[student.id];
    ...
    const mirrorState = entry?.mirror?.status === "ready"
      ...
      : entry?.mirror?.status === "error"
        ...
        : "Pendiente";
    const score = project?.audit?.score ? `${project.audit.score.percent}% (${project.audit.score.grade}/5)` : "—";
    const commitText = commit ? `${commit.shortSha} — ${commit.message}` : "—";
    return `| ${student.group} | ${student.name} | ${student.repository || "—"} | ${mirrorState} | ${score} | ${commitText} |`;
  }),
  "",
  "> Los puntajes son diagnósticos automáticos de evidencia técnica. La nota final requiere ejecución, explicación, modificación en vivo y autoría.",
  "",
];
...
console.log(
  `Monitor finalizado: ${result.repositoriesChecked} repositorios, ${result.mirrorsReady} copias, ${result.auditsReady} auditorías, ${result.updatesDetected} actualizaciones.`
);
